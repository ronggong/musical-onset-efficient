...
if [ -d /scratch/rgongcnnSyllableSeg_part1_jan_3channels ]; then
        rm -Rf /scratch/rgongcnnSyllableSeg_part1_jan_3channels
fi

# Second, replicate the structure of the experiment's folder:
# -----------------------------------------------------------
mkdir /scratch/rgongcnnSyllableSeg_part1_jan_3channels
mkdir /scratch/rgongcnnSyllableSeg_part1_jan_3channels/syllableSeg

#printf "Copying feature files into scratch directory...\n"
# Third, copy the experiment's data:
# ----------------------------------
#start=`date +%s`
#cp -rp /homedtic/rgong/cnnSyllableSeg/syllableSeg/feature_all_artist_filter_split.h5 /scratch/rgongcnnSyllableSeg_part1_jan_3channels/syllableSeg/
#end=`date +%s`

#printf "Finish copying feature files into scratch directory...\n"
#printf $((end-start))

python /homedtic/rgong/cnnSyllableSeg/jingjuSyllabicSegmentation/training_scripts/hpcDLScriptsSchluter/keras_cnn_syllableSeg_jan_madmom_original_3channels.py 1 4 8
