...
if __name__ == '__main__':
    ...
    for ii in range(8):
        ...
        filename_labels_train_validation_set = join(schluter_feature_data_path, 'temp', 'labels_train_set_all_jan_temp.pickle.gz')
        filename_sample_weights = join(schluter_feature_data_path, 'temp', 'sample_weights_all_jan_temp.pickle.gz')
        filename_scaler = join(schluter_feature_data_path, 'temp', 'scaler_jan_'+str(ii)+'.pickle.gz')
        ...
        # train the model
        file_path_model = '/homedtic/rgong/cnnSyllableSeg/out/schulter_jan_cv_'+str(ii)+'.h5'
        file_path_log = '/homedtic/rgong/cnnSyllableSeg/out/log/schulter_jan_cv_'+str(ii)+'.csv'
