...
def syllableSeg_jan_madmom_original_basecode(part, ii):
    ...
    feature_all, label_all, sample_weights_all, scaler = concatenateFeatureLabelSampleweights(train_fns,
                                                                                              ...
                                                                                              nlen=7,
                                                                                              scaling=True)
    filename_train_validation_set = join(bock_feature_data_path_madmom_positiveThreeSampleWeighting, 'temp', 'feature_all_jan_temp_' + str(ii) + '.h5')
    filename_labels_train_validation_set = join(bock_feature_data_path_madmom_positiveThreeSampleWeighting, 'temp', 'labels_train_set_all_jan_temp_' + str(ii) + '.pickle.gz')
    filename_sample_weights = join(bock_feature_data_path_madmom_positiveThreeSampleWeighting, 'temp', 'sample_weights_all_jan_temp_' + str(ii) + '.pickle.gz')
    filename_scaler = join(bock_feature_data_path_madmom_positiveThreeSampleWeighting, 'temp', 'scaler_jan_madmom_positiveThreeSampleWeighting_early_stopping_' + str(ii) + '.pickle.gz')
    ...
    # train the model
    file_path_model = '/homedtic/rgong/cnnSyllableSeg/out/schulter_jan_madmom_positiveThreeSampleWeighting_early_stopping_adam_cv_'+str(ii)+'.h5'
    file_path_log = '/homedtic/rgong/cnnSyllableSeg/out/log/schulter_jan_madmom_positiveThreeSampleWeighting_early_stopping_adam_cv_'+str(ii)+'.csv'

    # filename_train_validation_set_scratch = filename_train_validation_set
    # file_path_model = '../../temp/schulter_jan_madmom_positiveThreeSampleWeighting_cv_'+str(ii)+'.h5'
    # file_path_log = '../../temp/schulter_jan_madmom_positiveThreeSampleWeighting_cv_'+str(ii)+'.csv'
