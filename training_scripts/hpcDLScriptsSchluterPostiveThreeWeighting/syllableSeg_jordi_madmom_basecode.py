...
def syllableSeg_jordi_madmom_basecode(part, ii, model_name='jordi_timbral_schluter'):
    ...
    feature_all, label_all, sample_weights_all, scaler = concatenateFeatureLabelSampleweights(train_fns,
                                                                                              ...
                                                                                              nlen=7,
                                                                                              scaling=True)
    filename_train_validation_set = join(bock_feature_data_path_madmom_positiveThreeSampleWeighting, 'temp', 'feature_all_' + model_name + '_temp_' + str(ii) + '.h5')
    filename_labels_train_validation_set = join(bock_feature_data_path_madmom_positiveThreeSampleWeighting, 'temp', 'labels_train_set_all_' + model_name + '_temp_' + str(ii) + '.pickle.gz')
    filename_sample_weights = join(bock_feature_data_path_madmom_positiveThreeSampleWeighting, 'temp', 'sample_weights_all_' + model_name + '_temp_' + str(ii) + '.pickle.gz')
    filename_scaler = join(bock_feature_data_path_madmom_positiveThreeSampleWeighting, 'temp', 'scaler_' + model_name + '_madmom_positiveThreeSampleWeighting_early_stopping_' + str(ii) + '.pickle.gz')
