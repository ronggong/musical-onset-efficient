    ...
    for artist_name, recording_name in recordings:
        ...
        for idx, u_list in enumerate(nested_utterance_lists):
            ...
            if float(bpm[idx]):
                ...
                filename_mfcc_line = join(output_path,
                ...
                filename_label = join(output_path,
                ...
                filename_sample_weights = join(output_path,
...
def dump_feature_batch_onset_phrase(split='ismir', feature_type='mfccBands2D', train_test='train'):
    ...
    feature_data_jingju_phrase_path = join(feature_data_path, 'jingju_phrase')
    if not os.path.exists(feature_data_jingju_phrase_path):
        os.makedirs(feature_data_jingju_phrase_path)

    if len(trainNacta2017):

        mfcc_line_nacta2017 = \
            save_feature_label_sample_weights_onset_phrase(wav_path=nacta2017_wav_path,
                                                           textgrid_path=nacta2017_textgrid_path,
                                                           score_path=nacta2017_score_path,
                                                           recordings=nacta_data_2017,
                                                           feature_type=feature_type,
                                                           output_path=feature_data_jingju_phrase_path)
